...
var cmdSize = &cobra.Command{
	Use:   "tosize  [genesis.yaml] [config.yaml] [targetSize] [trieDBPath]",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		cfg.Genesis = gs
		targetSize, ok := big.NewInt(0).SetString(args[2], 10)
		if !ok || !targetSize.IsUint64() {
			return errors.Errorf("invalid target size %s", args[2])
		}
		if len(args) > 3 {
		...
		return checker.CheckIndexerToTargetSize(ctx,
			cs.StateFactory(),
			blockdao.TargetIndexer{
				Path:       cfg.Chain.TrieDBPath,
				TargetSize: targetSize.Uint64(),
			},
			func(height uint64) {
				if height%5000 == 0 {
